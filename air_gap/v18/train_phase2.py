...
def train_phase2():
    ...
    save_dir = "air_gap/v18/results_phase2"
    ...
    model.load_state_dict(torch.load("air_gap/v18/phase1_ae.pt"))
...
def evaluate(model, loader, device, epoch, save_dir, max_logged_samples=8):
    model.eval()
    metrics = {}
    logged_samples = []
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            for i in range(len(input_ids)):
                ...
                # Capture a handful of examples per epoch for later inspection.
    ...
    for t in sorted(metrics.keys()):
        if t == 'math':
            ...
            epoch_metrics["tasks"][t] = {"prec": prec, "rec": rec, "f1": f1, "total": total}
            print(f"  {t}: P={prec:.3f} R={rec:.3f} F1={f1:.3f}")

    # Persist compact metrics and a few examples per epoch.
    os.makedirs(save_dir, exist_ok=True)
    metrics_path = os.path.join(save_dir, "eval_metrics.jsonl")
    with open(metrics_path, "a") as f:
        f.write(json.dumps(epoch_metrics) + "\n")

    if logged_samples:
        samples_path = os.path.join(save_dir, f"eval_samples_epoch{epoch + 1}.jsonl")
