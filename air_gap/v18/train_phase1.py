...
def train_phase1():
    ...
    save_dir = "air_gap/v18/results_phase1"
    ...
    for epoch in range(epochs):
        ...
        eval_predictive_reconstruction(
            model,
            val_loader,
            device,
            epoch,
    ...
    torch.save(model.state_dict(), "air_gap/v18/phase1_ae.pt")
    print("Phase 1 Complete. Model saved.")

def eval_predictive_reconstruction(model, loader, device, epoch, eos_id, decoder_fn, save_dir):
    model.eval()
    ...
    # Persist compact epoch metric for later analysis (JSONL stays small).
    os.makedirs(save_dir, exist_ok=True)
    metrics_path = os.path.join(save_dir, "eval_metrics.jsonl")
    with open(metrics_path, "a") as f:
        f.write(json.dumps({"epoch": epoch + 1, "predictive_reconstruction_acc": predictive_accuracy}) + "\n")
