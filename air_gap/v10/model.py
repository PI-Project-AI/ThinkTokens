...
class VectorQuantizer(nn.Module):
    ...
    def forward(self, z, temp=1.0, hard=True):
        ...
        # hard: If True, use argmax (or straight-through Gumbel). If False, use soft Gumbel.
        
        z_flattened = z.reshape(-1, self.code_dim)
        
        # Calculate distances (negative logits)
        # d = ||z - e||^2 = ||z||^2 + ||e||^2 - 2 z.e
        # We can just use -d as logits.
        
        d = torch.sum(z_flattened ** 2, dim=1, keepdim=True) + \
            torch.sum(self.embedding.weight ** 2, dim=1) - \
            2 * torch.matmul(z_flattened, self.embedding.weight.t())
        
        logits = -d # [B*T, num_codes]
        
        if hard:
            # Standard VQ-VAE (Argmax + STE)
            ...
            # Gumbel-Softmax (Differentiable Soft Sampling)
            # Use gumbel_softmax from torch
            # logits: [B*T, num_codes]
            
            # Reparameterization trick
            # hard=True in gumbel_softmax returns one-hot but gradients flow via softmax
            # We want to anneal.
            # Usually we start with hard=False (Soft) and move to hard=True (ST-Gumbel) or standard VQ.
            # Let's support "Soft Gumbel" here.
            
            z_q_soft = F.gumbel_softmax(logits, tau=temp, hard=False, dim=-1)
            
            # z_q is weighted sum of codes
            z_q = torch.matmul(z_q_soft, self.embedding.weight).view(z.shape)
            
            # Loss? In Gumbel-VQ, we usually just have the task loss (and maybe KL to uniform).
            # But we can keep a small commitment loss to keep z close to code space?
            # Or just rely on task gradients moving z and embedding.
            # Let's add a small commitment loss to stabilize.
            
            loss = torch.mean((z_q.detach() - z) ** 2) # Only train encoder to target z_q?
            # Actually, standard Gumbel-Softmax doesn't need commitment loss necessarily,
            # but for VQ consistency let's keep it small.
...
class AirGapVQTransformer(nn.Module):
    def __init__(self, vocab_size, num_codes=512, code_dim=128, n_ir_tokens=20):
        super().__init__()
        self.num_codes = num_codes
        self.n_ir_tokens = n_ir_tokens
        
        self.reasoner_config = GPTConfig(vocab_size=vocab_size, n_embd=code_dim) 
        ...
        self.speaker_config = GPTConfig(vocab_size=vocab_size, n_embd=code_dim)
    ...
    def forward(self, input_ids, target_ids=None, vq_hard=True, vq_temp=1.0):
        ...
        z = x[:, -self.n_ir_tokens:, :] # [B, T_ir, D]
        
        # 2. VQ Step (The Bridge)
    ...
    def generate(self, input_ids, max_new_tokens=20, ir_mode='intact'):
        ...
        # 2. VQ Bridge
        z_q, _, vq_info = self.vq(z, hard=True) # Always hard for generation
