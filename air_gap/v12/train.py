...
def evaluate(model, loader, device, step, save_dir, dataset, final=False):
    ...
    limit = 300 if not final else len(loader.dataset)
    count = 0
    examples = []
    ...
    with torch.no_grad():
        for batch in loader:
            ...
            pred_ids, ir_indices = model.generate(input_ids, max_new_tokens=15, ir_mode='intact')
            ...
            for i in range(len(input_ids)):
                ...
                if len(examples) < 6: # 2 per task ideally
                    examples.append({
                        'task': task,
                        'input': input_texts[i],
                        'target': target_str,
    ...
    for t in tasks:
        ...
        print(f"  Task {t}: {acc:.2%} ({metrics[t]['correct']}/{tot})")
        log_entry[f'acc_{t}'] = acc
    
    # Compute Codebook Overlap (Jaccard)
    # Collect all codes per task
    # This requires running generation again or collecting during eval loop.
    # Let's reuse the examples? No, too few.
    # We need to track codes used per task.
    # Let's modify the loop above to track unique codes per task.
    
    # ... (Simpler to just skip overlap calc during training for speed, do it post-hoc or on small sample)
