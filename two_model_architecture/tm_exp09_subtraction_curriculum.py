"""
TM-EXP-09 (local): Factorized LA reasoner + subtraction curriculum vs text-AR baseline.

Idea:
- Keep TM-EXP-08 comparison scaffold and model family.
- Add a pre-registered subtraction-focused loss-weight schedule for LA training:
  examples with '-' operator receive higher loss weight (op1/op2 selectable).
"""
...
def scheduled_sub_weight(epoch: int, start: float, end: float, warmup_epochs: int) -> float:
    if warmup_epochs <= 1:
        return end
...
def train_epoch_la_factorized(
    ...
    loss_sum = 0.0
    acc_out_sum = 0.0
    mid_sub_frac_sum = 0.0
    out_sub_frac_sum = 0.0
    for batch in loader:
        ...
        mid_sub_mask = (x[:, 1] == minus_id).float()
        out_sub_mask = (x[:, 3] == minus_id).float()
        mid_w = torch.ones_like(mid_sub_mask)
        out_w = torch.ones_like(out_sub_mask)
        if sub_focus in {"mid", "both"}:
            mid_w = torch.where(mid_sub_mask > 0, torch.full_like(mid_w, sub_weight), mid_w)
        if sub_focus in {"out", "both"}:
            out_w = torch.where(out_sub_mask > 0, torch.full_like(out_w, sub_weight), out_w)

        mid_loss = (ce(mid_logits, y[:, 0]) * mid_w).mean()
        out_loss = (ce(out_logits, y[:, 1]) * out_w).mean()
        loss = mid_loss + out_loss
        opt.zero_grad()
        loss.backward()
        ...
        mid_sub_frac_sum += float(mid_sub_mask.mean().item())
        out_sub_frac_sum += float(out_sub_mask.mean().item())
        n += 1
    return {
        "loss": loss_sum / max(1, n),
        "acc_out": acc_out_sum / max(1, n),
        "mid_sub_frac": mid_sub_frac_sum / max(1, n),
        "out_sub_frac": out_sub_frac_sum / max(1, n),
    }
...
def run_seed(
    ...
    cfg: LAConfig,
    curriculum_cfg: Dict[str, object],
    seed: int,
    run_dir: str,
) -> Dict[str, object]:
    random.seed(seed)
    ...
    with open(metrics_path, "w", encoding="utf-8") as f:
        for epoch in range(1, cfg.epochs + 1):
            curr_sub_weight = scheduled_sub_weight(
                epoch=epoch,
                start=float(curriculum_cfg["sub_weight_start"]),
                end=float(curriculum_cfg["sub_weight_end"]),
                warmup_epochs=int(curriculum_cfg["sub_weight_warmup_epochs"]),
            )
            tr_la = train_epoch_la_factorized(
                la,
                train_loader,
                opt_la,
                sub_weight=curr_sub_weight,
                sub_focus=str(curriculum_cfg["sub_focus"]),
            )
                ...
                json.dumps(
                    {
                        ...
                        "train_la_curriculum_sub_weight": curr_sub_weight,
                        "train_la_mid_sub_frac": tr_la["mid_sub_frac"],
                        "train_la_out_sub_frac": tr_la["out_sub_frac"],
...
def main() -> None:
    parser = argparse.ArgumentParser(description="TM-EXP-09 factorized-LA subtraction curriculum vs text-AR")
    ...
    parser.add_argument("--la_sub_focus", type=str, default="both", choices=["mid", "out", "both"])
    parser.add_argument("--la_sub_weight_start", type=float, default=1.0)
    parser.add_argument("--la_sub_weight_end", type=float, default=2.5)
    parser.add_argument("--la_sub_weight_warmup_epochs", type=int, default=20)
    args = parser.parse_args()
    args.enforce_test_solution_coverage = bool(args.enforce_test_solution_coverage)
    if args.la_sub_weight_start < 1.0 or args.la_sub_weight_end < 1.0:
        raise ValueError("la_sub_weight_start/end must be >= 1.0")
    if args.la_sub_weight_warmup_epochs < 1:
        raise ValueError("la_sub_weight_warmup_epochs must be >= 1")
    ...
    curriculum_cfg = {
        "sub_focus": args.la_sub_focus,
        "sub_weight_start": args.la_sub_weight_start,
        "sub_weight_end": args.la_sub_weight_end,
        "sub_weight_warmup_epochs": args.la_sub_weight_warmup_epochs,
    }

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    run_dir = os.path.join(args.output_root, f"tm_exp_09_{ts}")
    os.makedirs(run_dir, exist_ok=True)

    prereg = {
        "experiment_id": "TM-EXP-09",
        "hypothesis": (
            "Subtraction-focused curriculum (loss upweight on '-' operators) improves "
            "factorized LA margin over text-AR on subtraction-heavy holdout."
        ),
        "acceptance_criteria": {
    ...
    for seed in parse_seeds(cfg.seeds):
        seed_summaries.append(run_seed(rows_by_split, cfg, curriculum_cfg, seed, run_dir))
